...
class BieuDoController extends Controller
{
    public function getSanPhamBanChay(Request $request)
    {
        ...
        $sanPhamID = SanPhamDonDatHang::whereYear('created_at', '=', $year)->whereMonth('created_at', '=', $month)->get();
        $hoaDon = DonDatHang::whereYear('created_at', '=', $year)->whereMonth('created_at', '=', $month)->where('trang_thai', 'hoa_don')->pluck('id')->toArray();
        $sanPhamID =  collect($sanPhamID);
        $sanPhamID = $sanPhamID->unique('san_pham_id')->pluck('san_pham_id')->toArray();
        $sanPhams = SanPhamDonDatHang::with('sanPham:id,ten_san_pham')->select('id', 'san_pham_id', 'doanh_thu')->whereIn('don_dat_hang_id', $hoaDon)->whereYear('created_at', '=', $year)->whereMonth('created_at', '=', $month)->get();
